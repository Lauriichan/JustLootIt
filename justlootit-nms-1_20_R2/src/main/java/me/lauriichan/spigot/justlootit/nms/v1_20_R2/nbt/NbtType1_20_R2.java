...
public final class NbtType1_20_R2<P, C extends Tag> {

    private static final Int2ObjectMap<NbtType1_20_R2<?, ?>> TYPES;

    static {
        Int2ObjectArrayMap<NbtType1_20_R2<?, ?>> map = new Int2ObjectArrayMap<>(12);
        ...
        create(map, TagType.LIST, ListTag.class, tag -> new ListTag1_20_R2<>(tag, TagType.getType(tag.getId())), tag -> ((ListTag1_20_R2<?>) tag).handle());
        create(map, TagType.COMPOUND, CompoundTag.class, CompoundTag1_20_R2::new, tag -> ((CompoundTag1_20_R2) tag).handle());
    ...
    private static <P, C extends Tag> void create(Int2ObjectMap<NbtType1_20_R2<?, ?>> map, TagType<P> type, Class<C> complexType,
        Function<C, P> complexToPrimitive, Function<P, C> primitiveToComplex) {
        map.put(type.tagId(), new NbtType1_20_R2<>(type, complexType, complexToPrimitive, primitiveToComplex));
    }

    @SuppressWarnings("unchecked")
    public static <T> NbtType1_20_R2<T, ?> get(TagType<T> type) {
        return (NbtType1_20_R2<T, ?>) TYPES.get(type.tagId());
    }
    ...
    private NbtType1_20_R2(TagType<P> type, Class<C> complexType, Function<C, P> complexToPrimitive, Function<P, C> primitiveToComplex) {
        this.type = type;
