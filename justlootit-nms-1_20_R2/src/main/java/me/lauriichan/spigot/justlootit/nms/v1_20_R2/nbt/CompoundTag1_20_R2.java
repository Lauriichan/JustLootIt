...
public final class CompoundTag1_20_R2 implements ICompoundTag {

    private final CompoundTag compoundTag;

    public CompoundTag1_20_R2() {
        this(new CompoundTag());
    }

    public CompoundTag1_20_R2(CompoundTag compoundTag) {
    ...
    public ICompoundTag getCompound(String key) {
        if (compoundTag.contains(key, TagType.COMPOUND.tagId())) {
            return new CompoundTag1_20_R2((CompoundTag) compoundTag.get(key));
        }
        return null;
    }
    ...
    public IListTag<?> getList(String key) {
        if (compoundTag.contains(key, TagType.LIST.tagId())) {
            ...
            return new ListTag1_20_R2<>(listTag, TagType.getType(listTag.getElementType()));
        }
        return null;
    }
    ...
    public <T> IListTag<T> getList(String key, TagType<T> type) {
        if (compoundTag.contains(key, TagType.LIST.tagId())) {
            ...
            if (listType == type.tagId() || listType == 0) {
                return new ListTag1_20_R2<>(listTag, type);
            }
        }
        return null;
    }
    ...
    public void set(String key, ICompoundTag compound) {
        compoundTag.put(key, ((CompoundTag1_20_R2) compound).handle());
    }

    @Override
    public void set(String key, IListTag<?> list) {
        compoundTag.put(key, ((ListTag1_20_R2<?>) list).handle());
    }
    ...
    @Override
    public void clear() {
        NmsHelper1_20_R2.clearCompound(compoundTag);
    }
