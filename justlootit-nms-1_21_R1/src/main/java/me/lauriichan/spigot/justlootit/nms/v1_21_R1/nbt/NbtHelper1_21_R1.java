...
public final class NbtHelper1_21_R1 extends NbtHelper {

    @Override
    public ICompoundTag createCompound() {
        return new CompoundTag1_21_R1();
    }

    @Override
    public <T> IListTag<T> createList(TagType<T> type) {
        return new ListTag1_21_R1<>(type);
    }
    
    @Override
    public ICompoundTag asTag(ItemStack itemStack) {
        return new CompoundTag1_21_R1(ItemStackIO1_21_R1.ITEM_STACK.asNbt(itemStack));
    }
    ...
    public ICompoundTag getCustomDataTag(ItemStack itemStack) {
        ...
        CompoundTag tag = NmsHelper1_21_R1.getCustomData(itemStack.getItemMeta());
        if (tag == null) {
            NmsHelper1_21_R1.setCustomData(meta, tag = new CompoundTag());
        }
        return new CompoundTag1_21_R1(tag);
    }
    
    @Override
    public ItemStack asItem(ICompoundTag tag) {
        return ItemStackIO1_21_R1.ITEM_STACK.fromNbt(((CompoundTag1_21_R1) tag).handle());
    }
