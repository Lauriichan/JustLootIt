...
public class ProtoChunk1_21_R1 extends ProtoChunk {

    private final ProtoWorld1_21_R1 world;
    ...
    public ProtoChunk1_21_R1(ProtoWorld1_21_R1 world, net.minecraft.world.level.chunk.ProtoChunk chunk, int x, int z) {
        this.world = world;
        this.chunk = chunk;
        this.x = x;
        this.z = z;
        for (Map.Entry<BlockPos, CompoundTag> entry : chunk.getBlockEntityNbts().entrySet()) {
            blockEntities.put(packShort(entry.getKey()),
                new ProtoBlockEntity1_21_R1(world.registry(), entry.getKey(), chunk.getBlockState(entry.getKey()), entry.getValue()));
        }
        for (CompoundTag entityTag : chunk.getEntities()) {
            entities.add(new ProtoEntity1_21_R1(entityTag));
        }
    }
    ...
    @Override
    public ProtoWorld1_21_R1 getWorld() {
        return world;
    }
    ...
    public ProtoBlock getBlock(Vec3i vector) {
        ...
        return new ProtoBlock1_21_R1(pos, chunk.getBlockState(pos));
    }
    ...
    public ProtoBlockEntity asBlockEntity(ProtoBlock protoBlock) {
        ...
        return blockEntities.get(packShort(((ProtoBlock1_21_R1) protoBlock).pos()));
    }

    @Override
    public void updateBlock(ProtoBlock protoBlock) {
        if (protoBlock instanceof ProtoBlock1_21_R1 block) {
            ...
            if (state.getBlock() instanceof EntityBlock) {
                ...
                if (!blockEntities.containsKey(id)) {
                    ...
                    ProtoBlockEntity1_21_R1 blockEntity = new ProtoBlockEntity1_21_R1(world.registry(), pos, state, tag);
                    blockEntities.put(id, blockEntity);
                }
            }
        } else if (protoBlock instanceof ProtoBlockEntity1_21_R1 blockEntity) {
    ...
    public void updateEntity(ProtoEntity entity) {
        ((ProtoEntity1_21_R1) entity).save();
        dirty = true;
    }
