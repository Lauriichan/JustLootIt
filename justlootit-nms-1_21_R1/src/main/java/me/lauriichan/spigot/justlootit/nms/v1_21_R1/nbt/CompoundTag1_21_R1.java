...
public final class CompoundTag1_21_R1 implements ICompoundTag {

    private final CompoundTag compoundTag;

    public CompoundTag1_21_R1() {
        this(new CompoundTag());
    }

    public CompoundTag1_21_R1(CompoundTag compoundTag) {
    ...
    public ICompoundTag getCompound(String key) {
        if (compoundTag.contains(key, TagType.COMPOUND.tagId())) {
            return new CompoundTag1_21_R1((CompoundTag) compoundTag.get(key));
        }
        return null;
    }
    ...
    public <T> IListTag<T> getList(String key, TagType<T> type) {
        if (compoundTag.contains(key, TagType.LIST.tagId())) {
            ...
            if (listType == type.tagId() || listType == 0) {
                return new ListTag1_21_R1<>(listTag, type);
            }
        }
        return null;
    }
    ...
    public void set(String key, ICompoundTag compound) {
        compoundTag.put(key, ((CompoundTag1_21_R1) compound).handle());
    }

    @Override
    public void set(String key, IListTag<?> list) {
        compoundTag.put(key, ((ListTag1_21_R1<?>) list).handle());
    }
    ...
    @Override
    public void clear() {
        NmsHelper1_21_R1.clearCompound(compoundTag);
    }
