...
public final class CompoundTag1_21_R3 implements ICompoundTag {

    private final CompoundTag compoundTag;

    public CompoundTag1_21_R3() {
        this(new CompoundTag());
    }

    public CompoundTag1_21_R3(CompoundTag compoundTag) {
    ...
    public ICompoundTag getCompound(String key) {
        if (compoundTag.contains(key, TagType.COMPOUND.tagId())) {
            return new CompoundTag1_21_R3((CompoundTag) compoundTag.get(key));
        }
        return null;
    }
    ...
    public IListTag<?> getList(String key) {
        if (compoundTag.contains(key, TagType.LIST.tagId())) {
            ...
            return new ListTag1_21_R3<>(listTag, TagType.getType(listTag.getElementType()));
        }
        return null;
    }
    ...
    public <T> IListTag<T> getList(String key, TagType<T> type) {
        if (compoundTag.contains(key, TagType.LIST.tagId())) {
            ...
            if (listType == type.tagId() || listType == 0) {
                return new ListTag1_21_R3<>(listTag, type);
            }
        }
        return null;
    }
    ...
    public void set(String key, ICompoundTag compound) {
        compoundTag.put(key, ((CompoundTag1_21_R3) compound).handle());
    }
