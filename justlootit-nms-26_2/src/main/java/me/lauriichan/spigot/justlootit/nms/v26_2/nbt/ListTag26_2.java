...
public final class ListTag26_2<T> implements IListTag<T> {

    private static final class SimpleIterator<T> implements Iterator<T> {

        private final ListTag26_2<T> tag;
        private volatile int index;

        public SimpleIterator(final ListTag26_2<T> tag) {
            this.tag = tag;
        }
    ...
    private final NbtType26_2<T, ?> type;

    public ListTag26_2(TagType<T> type) {
        this(new ListTag(), type);
    }

    public ListTag26_2(ListTag listTag, TagType<T> type) {
        this.listTag = listTag;
        this.type = NbtType26_2.get(type);
    }
