package me.lauriichan.spigot.justlootit.nms.v26_2.nbt;
...
public final class NbtType26_2<P, C extends Tag> {

    private static final Int2ObjectMap<NbtType26_2<?, ?>> TYPES;

    static {
        Int2ObjectArrayMap<NbtType26_2<?, ?>> map = new Int2ObjectArrayMap<>(12);
        ...
        create(map, TagType.LIST, ListTag.class, tag -> new ListTag26_2<>(tag, TagType.getType(tag.getId())), tag -> ((ListTag26_2<?>) tag).handle());
        create(map, TagType.COMPOUND, CompoundTag.class, CompoundTag26_2::new, tag -> ((CompoundTag26_2) tag).handle());
    ...
    private static <P, C extends Tag> void create(Int2ObjectMap<NbtType26_2<?, ?>> map, TagType<P> type, Class<C> complexType,
        Function<C, P> complexToPrimitive, Function<P, C> primitiveToComplex) {
        map.put(type.tagId(), new NbtType26_2<>(type, complexType, complexToPrimitive, primitiveToComplex));
    }

    @SuppressWarnings("unchecked")
    public static <T> NbtType26_2<T, ?> get(TagType<T> type) {
        return (NbtType26_2<T, ?>) TYPES.get(type.tagId());
    }
    ...
    private NbtType26_2(TagType<P> type, Class<C> complexType, Function<C, P> complexToPrimitive, Function<P, C> primitiveToComplex) {
        this.type = type;
