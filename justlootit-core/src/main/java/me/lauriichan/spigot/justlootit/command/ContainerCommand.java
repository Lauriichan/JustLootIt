...
public class ContainerCommand implements ICommandExtension {
    ...
    private void runCreate(final JustLootItPlugin plugin, final Actor<?> actor, final ContainerType type, final Coord x, final Coord y, final Coord z, World world, final boolean useEntity) {
    ...
    private void createEntityContainer(final JustLootItPlugin plugin, final Actor<?> actor, final ContainerType type,
        ...
        type.entityCreator().function().create(plugin, actor, closest.getLocation(), closest, (creator) -> {
            plugin.versionHandler().getLevel(location.getWorld()).getCapability(StorageCapability.class).ifPresentOrElse(capability -> {
                ...
                actor.sendTranslatedMessage(Messages.COMMAND_CONTAINER_CREATE_SUCCESS_ENTITY,
                    Key.of("x", location.getBlockX()), Key.of("y", location.getBlockY()), Key.of("z", location.getBlockZ()),
                    Key.of("world", location.getWorld().getName()), Key.of("type", type), Key.of("id", id));
    ...
    private void createBlockContainer(final JustLootItPlugin plugin, final Actor<?> actor, final ContainerType type, final Location location) {
        ...
        if (block.isEmpty()) {
            actor.sendTranslatedMessage(Messages.COMMAND_CONTAINER_CREATE_NOT_FOUND_BLOCK,
                Key.of("x", location.getBlockX()), Key.of("y", location.getBlockY()), Key.of("z", location.getBlockZ()),
        ...
        if (!(state instanceof org.bukkit.block.Container stateContainer)) {
            actor.sendTranslatedMessage(Messages.COMMAND_CONTAINER_CREATE_UNSUPPORTED_BLOCK,
                Key.of("x", location.getBlockX()), Key.of("y", location.getBlockY()), Key.of("z", location.getBlockZ()),
                Key.of("world", location.getWorld()), Key.of("type", type));
            return;
        }
        type.blockCreator().function().create(plugin, actor, location, stateContainer, (creator) -> {
            plugin.versionHandler().getLevel(location.getWorld()).getCapability(StorageCapability.class).ifPresentOrElse(capability -> {
                ...
                long id = storage.newId();
                storage.write(creator.apply(id));
                actor.sendTranslatedMessage(Messages.COMMAND_CONTAINER_CREATE_SUCCESS_BLOCK,
                    Key.of("x", location.getBlockX()), Key.of("y", location.getBlockY()), Key.of("z", location.getBlockZ()),
                    Key.of("world", location.getWorld().getName()), Key.of("type", type), Key.of("id", id));
            }, () -> actor.sendTranslatedMessage(Messages.COMMAND_SYSTEM_ERROR_STORAGE_ACCESS_LEVEL, Key.of("level", location.getWorld().getName())));
        });
    ...
    private void runGroup(final JustLootItPlugin plugin, final Actor<?> actor, final RefreshGroup group, final Coord x, final Coord y, final Coord z,
        final World world) {
        final Location loc = CommandUtil.getLocation(actor, x, y, z, world);
        plugin.scheduler().regional(loc, () -> {
            ...
            doGroupForBlockContainer(plugin, block, stateContainer, loc.getWorld(), actor, loc.getBlockX(), loc.getBlockY(), loc.getBlockZ(), group);
        });
    }
    ...
    private void doGroupForEntityContainer(JustLootItPlugin plugin, World world, Actor<?> actor, int x, int y, int z, RefreshGroup group) {
        ...
        if (entities.isEmpty()) {
            actor.sendTranslatedMessage(Messages.COMMAND_CONTAINER_GROUP_NO_CONTAINER_ENTITY, Key.of("x", x),
        ...
        List<Entity> validEntities = entities.stream()
            .filter(entity -> EntityUtil.isSuppportedEntity(entity) || EntityUtil.isItemFrame(entity)).toList();
        if (validEntities.isEmpty()) {
            actor.sendTranslatedMessage(Messages.COMMAND_CONTAINER_GROUP_NO_CONTAINER_ENTITY, Key.of("x", x), Key.of("y", y), Key.of("z", z),
        ...
        if (!dataContainer.has(JustLootItKey.identity(), PersistentDataType.LONG)) {
            actor.sendTranslatedMessage(Messages.COMMAND_CONTAINER_GROUP_NO_CONTAINER_ENTITY, Key.of("x", loc.getX()),
        ...
        plugin.versionHandler().getLevel(world).getCapability(StorageCapability.class).ifPresentOrElse(capability -> {
            ...
            if (group == null) {
                ...
                actor.sendTranslatedMessage(Messages.COMMAND_CONTAINER_GROUP_SET_ENTITY, Key.of("x", loc.getX()),
                    Key.of("y", loc.getY()), Key.of("z", loc.getZ()), Key.of("world", world.getName()), Key.of("group", group.id()));
            }
        }, () -> actor.sendTranslatedMessage(Messages.COMMAND_SYSTEM_ERROR_STORAGE_ACCESS_LEVEL, Key.of("level", world.getName())));
    }

    private void doGroupForBlockContainer(JustLootItPlugin plugin, Block block, org.bukkit.block.Container stateContainer, World world, Actor<?> actor, int x, int y, int z, RefreshGroup group) {
        ...
        if (!dataContainer.has(JustLootItKey.identity(), PersistentDataType.LONG)
            ...
            if (block.isEmpty()) {
                actor.sendTranslatedMessage(Messages.COMMAND_CONTAINER_GROUP_NO_CONTAINER_BLOCK, Key.of("x", x), Key.of("y", y),
                    Key.of("z", z), Key.of("world", world.getName()));
                dataContainer.remove(JustLootItKey.chestData());
                stateContainer.update(false, false);
                return;
        ...
        if (!dataContainer.has(JustLootItKey.identity(), PersistentDataType.LONG)) {
            actor.sendTranslatedMessage(Messages.COMMAND_CONTAINER_GROUP_NO_CONTAINER_BLOCK, Key.of("x", location.getBlockX()),
        ...
        plugin.versionHandler().getLevel(world).getCapability(StorageCapability.class).ifPresentOrElse(capability -> {
            ...
            if (group == null) {
                ...
                actor.sendTranslatedMessage(Messages.COMMAND_CONTAINER_GROUP_SET_BLOCK, Key.of("x", location.getX()),
                    Key.of("y", location.getY()), Key.of("z", location.getZ()), Key.of("world", world.getName()), Key.of("group", group.id()));
            }
        }, () -> actor.sendTranslatedMessage(Messages.COMMAND_SYSTEM_ERROR_STORAGE_ACCESS_LEVEL, Key.of("level", world.getName())));
    }
