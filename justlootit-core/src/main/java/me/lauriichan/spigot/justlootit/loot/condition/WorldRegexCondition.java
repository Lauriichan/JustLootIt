...
public record WorldRegexCondition(String stringPattern, Predicate<String> predicate) implements ILootCondition {

    @Override
    public boolean includes(Container container, PlayerAdapter player, Location location, NamespacedKey lootTableKey) {
        return lootTableKey != null && predicate.test(location.getWorld().getName());
    }
