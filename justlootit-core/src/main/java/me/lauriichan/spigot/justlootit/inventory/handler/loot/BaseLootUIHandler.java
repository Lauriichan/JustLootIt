...
public abstract class BaseLootUIHandler implements IHandler {
    ...
    protected final JustLootItPlugin plugin = JustLootItPlugin.get();
    protected final VersionHandler versionHandler = plugin.versionHandler();
    ...
    public boolean onEventClose(final HumanEntity entity, final IGuiInventory inventory) {
        ...
        try {
            final Long id = inventory.attr(ATTR_ID, Long.class);
            if (id == null) {
                return false;
            }
            ...
            player.getCapability(StorageCapability.class).ifPresent(capability -> {
                ...
                try {
                    ...
                    plugin.logger().error("Failed to save cached inventory with id {0} for player {1} ({2})", throwable, id,
