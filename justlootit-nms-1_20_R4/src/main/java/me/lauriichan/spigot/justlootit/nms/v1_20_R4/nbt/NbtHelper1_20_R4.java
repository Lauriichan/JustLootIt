...
public final class NbtHelper1_20_R4 extends NbtHelper {

    @Override
    public ICompoundTag createCompound() {
        return new CompoundTag1_20_R4();
    }

    @Override
    public <T> IListTag<T> createList(TagType<T> type) {
        return new ListTag1_20_R4<>(type);
    }
    
    @Override
    public ICompoundTag asTag(ItemStack itemStack) {
        return new CompoundTag1_20_R4(ItemStackIO1_20_R4.ITEM_STACK.asNbt(itemStack));
    }
    ...
    public ICompoundTag getCustomDataTag(ItemStack itemStack) {
        ...
        CompoundTag tag = NmsHelper1_20_R4.getCustomData(itemStack.getItemMeta());
        if (tag == null) {
            NmsHelper1_20_R4.setCustomData(meta, tag = new CompoundTag());
        }
        return new CompoundTag1_20_R4(tag);
    }
    
    @Override
    public ItemStack asItem(ICompoundTag tag) {
        return ItemStackIO1_20_R4.ITEM_STACK.fromNbt(((CompoundTag1_20_R4) tag).handle());
    }
