package me.lauriichan.spigot.justlootit.nms.v1_20_R4.convert;
...
public class ProtoChunk1_20_R4 extends ProtoChunk {

    private final ProtoWorld1_20_R4 world;
    ...
    public ProtoChunk1_20_R4(ProtoWorld1_20_R4 world, net.minecraft.world.level.chunk.ProtoChunk chunk, int x, int z) {
        this.world = world;
        this.chunk = chunk;
        this.x = x;
        this.z = z;
        for (Map.Entry<BlockPos, CompoundTag> entry : chunk.getBlockEntityNbts().entrySet()) {
            blockEntities.put(packShort(entry.getKey()),
                new ProtoBlockEntity1_20_R4(world.registry(), entry.getKey(), chunk.getBlockState(entry.getKey()), entry.getValue()));
        }
        for (CompoundTag entityTag : chunk.getEntities()) {
            entities.add(new ProtoEntity1_20_R4(entityTag));
        }
    }
    ...
    @Override
    public ProtoWorld1_20_R4 getWorld() {
        return world;
    }
    ...
    public ProtoBlock getBlock(Vec3i vector) {
        ...
        return new ProtoBlock1_20_R4(pos, chunk.getBlockState(pos));
    }
    ...
    public ProtoBlockEntity asBlockEntity(ProtoBlock protoBlock) {
        ...
        return blockEntities.get(packShort(((ProtoBlock1_20_R4) protoBlock).pos()));
    }
        ...
        if (protoBlock instanceof ProtoBlock1_20_R4 block) {
            ...
            if (state.getBlock() instanceof EntityBlock) {
                ...
                if (!blockEntities.containsKey(id)) {
                    ...
                    ProtoBlockEntity1_20_R4 blockEntity = new ProtoBlockEntity1_20_R4(world.registry(), pos, state, tag);
    ...
    public void updateEntity(ProtoEntity entity) {
        ((ProtoEntity1_20_R4) entity).save();
        dirty = true;
    }
