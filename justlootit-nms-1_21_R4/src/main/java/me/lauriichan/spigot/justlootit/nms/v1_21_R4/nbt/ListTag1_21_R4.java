...
public final class ListTag1_21_R4<T> implements IListTag<T> {

    private static final class SimpleIterator<T> implements Iterator<T> {

        private final ListTag1_21_R4<T> tag;
        private volatile int index;

        public SimpleIterator(final ListTag1_21_R4<T> tag) {
            this.tag = tag;
        }
    ...
    private final NbtType1_21_R4<T, ?> type;

    public ListTag1_21_R4(TagType<T> type) {
        this(new ListTag(), type);
    }

    public ListTag1_21_R4(ListTag listTag, TagType<T> type) {
        this.listTag = listTag;
        this.type = NbtType1_21_R4.get(type);
    }
