...
public class ProtoChunk26_3 extends ProtoChunk {

    private final ProtoWorld26_3 world;
    ...
    public ProtoChunk26_3(ProtoWorld26_3 world, net.minecraft.world.level.chunk.ProtoChunk chunk, int x, int z) {
        this.world = world;
        this.chunk = chunk;
        this.x = x;
        this.z = z;
        for (Map.Entry<BlockPos, CompoundTag> entry : chunk.getBlockEntityNbts().entrySet()) {
            blockEntities.put(packShort(entry.getKey()),
                new ProtoBlockEntity26_3(world.registry(), entry.getKey(), chunk.getBlockState(entry.getKey()), entry.getValue()));
        }
        for (CompoundTag entityTag : chunk.getEntities()) {
            entities.add(new ProtoEntity26_3(world.registry(), entityTag));
        }
    }
    ...
    @Override
    public ProtoWorld26_3 getWorld() {
        return world;
    }
    ...
    public ProtoBlock getBlock(Vec3i vector) {
        ...
        return new ProtoBlock26_3(pos, chunk.getBlockState(pos));
    }
    ...
    public ProtoBlockEntity asBlockEntity(ProtoBlock protoBlock) {
        ...
        return blockEntities.get(packShort(((ProtoBlock26_3) protoBlock).pos()));
    }

    @Override
    public void updateBlock(ProtoBlock protoBlock) {
        if (protoBlock instanceof ProtoBlock26_3 block) {
            ...
            if (state.getBlock() instanceof EntityBlock) {
                ...
                if (!blockEntities.containsKey(id)) {
                    ...
                    ProtoBlockEntity26_3 blockEntity = new ProtoBlockEntity26_3(world.registry(), pos, state, tag);
                    blockEntities.put(id, blockEntity);
                }
            }
        } else if (protoBlock instanceof ProtoBlockEntity26_3 blockEntity) {
