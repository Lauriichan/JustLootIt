package me.lauriichan.spigot.justlootit.nms.v1_21_R7.io;
...
public abstract class NbtIO1_21_R7<E, N extends Tag> extends IOHandler<E> {
    ...
    private static final int SERVER_VERSION = SharedConstants.getCurrentVersion().dataVersion().version();
    ...
    public NbtIO1_21_R7(final Class<E> type, final TagType<N> tagType) {
        super(type);
    ...
    private Result<N> readTag(DataInputStream stream) throws IOException {
        ...
        int tagVersion = compound.getIntOr(VERSION_ID, -1);
        if (tagVersion == -1) {
    ...
    private Result<ListTag> readListTag(DataInputStream stream, boolean readListDirectly) throws IOException {
        if (readListDirectly) {
            ...
            TagType<?> elementTagType = TagTypes.getType(listTag.identifyRawElementType());
